...
def _project_all_points_inside_walkable_area(
    ...
    """Checks every point, that is considered to invalid.
    ...
    Args:
        traj_data (pedpy.TrajectoryData): The trajectory data to be tested and
            corrected
        geo_data (pd.DataFrame):  the walkable area is converted into a pd.Dataframe: type : wall_type,
...
def _project_single_inside_walkable_area(
    ...
    for row in geo_data.itertuples(index=False):
        ...
        if row.type == WallType.walls:  # first, testing all the sides, which are walls.
        ...
        elif row.type == WallType.obstacle:  # testing obstacles
            ...
            raise (InputError(f"Not supported geometry type: {row.type}"))
...
def _push_out_of_wall(
    ...
    Args:
        ...
        geometry (pd.Dataframe):  the walkable area is converted into a pd.Dataframe: type : wall_type,
            direction : int, points : list  (lists like [p1x, p1y, p2x, p2y], describing  each
            one edge of the wall).
...
def _calculate_movement_wall(
    ...
    Returns:
         The moved x, y coordinates, if they had to be adjusted. Else the original x,y coordinates.
    """
    if back_distance_wall <= distance <= max_distance_wall:
...
def _calculate_movement_obstacle(
    ...
    Returns:
         The moved x, y coordinates, if they had to be adjusted. Else the original x,y coordinates.
    """
    if back_distance_obst <= distance <= max_distance_obst:
        # x' = (x-a)*((c-b)/(c-a))+b
...
def _calculate_movement_adjusting(
    ...
    Returns:
         The moved x, y coordinates, if they had to be adjusted. Else the original x,y coordinates.
    """
    if back_distance_obst <= distance <= min_distance_obst:
...
def _move_from_wall(
    ...
    """Moves the points out of the obstacles/walls by using different methods of linear algebra.
    ...
    a_sq = _distance_squared(p1x, p1y, p2x, p2y)
    b_sq = _distance_squared(p1x, p1y, x, y)
    c_sq = _distance_squared(x, y, p2x, p2y)
    x_sq = ((a_sq + b_sq - c_sq) * (a_sq + b_sq - c_sq)) / (4 * a_sq)
    ...
    vect_b = [x - p1x, y - p1y]
    cos_p = np.dot(vect_a, vect_b) / (npl.norm(vect_a) * npl.norm(vect_b))
    dist_x = math.sqrt(x_sq) * np.sign(cos_p)
    dist_a = math.sqrt(a_sq)
    ...
    if h == 0:
        ...
        x_sq = ((a_sq + b_sq - c_sq) * (a_sq + b_sq - c_sq)) / (4 * a_sq)
        h = _distance_from_wall(x, y, p1x, p1y, p2x, p2y, direction)
        dist_x = math.sqrt(math.fabs(x_sq))
...
def _point_valid_test(
    ...
    if not point_is_valid:
        for row in geo_data.itertuples(index=False):
            direction = row.direction
            for edge in row.points:
                p1x, p1y, p2x, p2y = edge
                if _close_from_wall(x, y, p1x, p1y, p2x, p2y):
                    distance = _distance_from_wall(x, y, p1x, p1y, p2x, p2y, direction)
                    if row.type == WallType.obstacle:
                        x, y = _calculate_movement_obstacle(
                            edge, direction, distance, back_distance_obst, max_distance_obst, min_distance_obst, x, y
                        )
                    elif row.type == WallType.walls:
                        x, y = _calculate_movement_wall(
                            edge, direction, distance, back_distance_wall, max_distance_wall, min_distance_wall, x, y
                        )
                    else:
...
def _convert_walk_area_into_geo_data(walk_area: WalkableArea) -> pd.DataFrame:
    """Converts WalkableArea to pd.Dataframe.
    ...
    for obstacle in obstacle_list:
        ...
        while i < len(obstacle) - 1:
            ...
            i = i + 1
        geometry_dataframe.loc[len(geometry_dataframe)] = [
            WallType.obstacle,
            -1 * _compute_direction(vertices),
            vertices,
        ]
    ...
    while i < len(wall_coords) - 1:
        ...
        vertices.append(vertice)
        i = i + 1
    geometry_dataframe.loc[len(geometry_dataframe)] = [WallType.walls, _compute_direction(vertices), vertices]
    return geometry_dataframe


def _compute_direction(vertices: list[list[float]]) -> float:
    """Calculates, whether the following points are initialized clockwise or counterclockwise.
    ...
    while i < len(vertices):
        ...
        angle = np.sign(sin_p) * np.arccos(cos_p)
        total_angle += angle
        i = i + 1
        j = (i + 1) % len(vertices)
...
def _close_from_wall_triangle(x: float, y: float, p1x: float, p1y: float, p2x: float, p2y: float) -> int:
    a = np.array([p2x - p1x, p2y - p1y])
    b = np.array([x - p1x, y - p1y])
    c = np.array([x - p2x, y - p2y])
    a = np.abs(a)
    b = np.abs(b)
    c = np.abs(c)
