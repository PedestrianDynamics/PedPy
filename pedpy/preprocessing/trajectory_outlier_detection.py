...
def detect_outliers(
    ...
    for i in range(len(all_single_trajectories)):
        ...
        if critical is None: critical = int(0.1 * len(trajectory_single))
        ...
        if len(outliers) > 0:
            if (len(list(chain.from_iterable(outliers))) > percentage_invalid * len(trajectory_single)
                    or deal==DealTrajectory.delete):
                _log.warning(f"Trajectory with personID {i + 1} has to many invalid points "
                             f"and cannot be corrected")
            ...
            match deal:
                case DealTrajectory.delete:
                    if deleting:
                        ...
                        continue
                    else: _log.info(f"Trajectory with personID {i + 1} will be returned unchanged")
...
def _check_parameters(
    ...
    if tolerance < 1:
        raise InputError(f"The tolerance has to be greater then 0 for meaningful results output")
    if quantile < 0.5 or quantile >= 1:
        raise InputError(f"The quantile of all distances between following points, that is used "
                         f"as a guideline for the expected distance between two points to locate "
                         f"outlier should be lesser that one 1 but at least 0.5")
    if percentage_invalid > 100 or percentage_invalid < 0:
        raise InputError(f"A value for a percentage has to be between 0 and 100")
    if max_length <= 1:
        raise InputError(f"The maximum length of an outlier has to be positive and greater than 1 "
...
def _detect_single_outliers(
    ...
    for i in range(len(distances)):
        ...
        if len(outliers) > 0 and outliers[-1][-1] + 1 >= i: continue
        if distances[i] > tolerance * quantile: #Testing, whether the distance between to points is abnormal -> sign of outlier
            following_outliers = []
            if i == 1:
                ...
                if _outliers_beginning(trajectory_single, quantile, tolerance):
                    ...
                    if i + 1 < len(distances):
                        _detect_multiple_outliers(
                            quantile, trajectory_single, following_outliers)
                else:
                    following_outliers.append(0)
            else:
                following_outliers.append(i)
                if i+1 < len(distances): _detect_multiple_outliers(quantile, trajectory_single, following_outliers)
            ...
            if following_outliers[0]<critical and len(following_outliers)> max_length:
                ...
                if following_outliers[0] == 0 and len(outliers) > 0 and outliers[0] == [0]:
                    ...
                    del outliers[0]
                outliers = [
                    group for group in outliers
                    if not any(x in following_outliers for x in group)
                ]
            ...
            # -> are considered outliers, because the index list it not long enough to count as a trajectory
                ...
                if following_outliers[0] < critical: deal = DealTrajectory.delete
                if drops_only:
                    if len(outliers)>0:
                        outliers = [group for group in outliers
                                    if 0 in group]
                        outliers.append(following_outliers)
    ...
    if drops_only and len(outliers)>0:
        outliers =[group for group in outliers
                    if 0 in group
                   or len(distances)-1 in group]
        deal = DealTrajectory.split
    return outliers, deal

def _detect_multiple_outliers(
        quantile: float,
        traj_data: pd.DataFrame,
        following_outliers: list[int]
)->None:
    """
    ...
    """
    correct_point = np.array([traj_data.iloc[following_outliers[-1] - 1]["x"], traj_data.iloc[following_outliers[-1] - 1]["y"]])  # The last correct point is the point before the outlier
    following_point = np.array([traj_data.iloc[following_outliers[-1] + 1]["x"], traj_data.iloc[following_outliers[-1] + 1]["y"]]) # The outlier is already tested and documented, so we'll proceed with the point afterward
...
def _outliers_beginning(
    ...
    if len(index_outliers)%2: #Uneven Number, first or last point is an outlier, not both
        ...
        return len(trajectory_single)-1 not in index_outliers  #if the last step of the person is an outlier, there is
        # still an even number of outliers even when the first point is incorrect (an outlier)

def _traj_to_xy(
        single_traj_data: pd.DataFrame
)->np.ndarray:
    return np.column_stack(
        [single_traj_data["x"].to_numpy(), single_traj_data["y"].to_numpy()])

def _direction_of_trajectory(
        points:np.ndarray
)->np.ndarray:
...
def _correct_trajectory(
    ...
    for following_outliers in outliers:
        ...
        single_trajectory = _correct_outliers(single_trajectory, following_outliers,
                                              median, mean_direction)

    return single_trajectory#

def _correct_outliers(
        traj_single:pd.DataFrame,
    ...
    if outlier_index[-1] +1 == len(traj_single):
        ...
        last_correct_point = np.array([traj_single.iloc[outlier_index[0] - 1]["x"], traj_single.iloc[outlier_index[0] - 1][ "y"]])
        first_correct_point = np.array([traj_single.iloc[outlier_index[-1] + 1]["x"], traj_single.iloc[outlier_index[-1] + 1][ "y"]])
...
def _false_points_at_end(
    ...
    last_correct_point = np.array([traj_single.iloc[outlier_index[0] - 1]["x"],
                                   traj_single.iloc[outlier_index[0] - 1]["y"]])
    last_traj_point = last_correct_point + mean_direction * len(outlier_index) * median
    insert_points = np.linspace( last_correct_point, last_traj_point,
                                len(outlier_index) - 1)
...
def _false_points_at_beginning(
    ...
    last_correct_point = np.array([traj_single.iloc[outlier_index[-1]+1]["x"],
                                   traj_single.iloc[outlier_index[-1]+1][ "y"]])
...
def _to_traj_data(
        original_traj_data: TrajectoryData,
        single_trajectories_to_keep: list[pd.DataFrame],
    ...
    for i in range(len(single_trajectories_to_keep)):
        ...
        if single_trajectories_to_keep[i].iloc[0,0] != i+1: # i+1 corresponds to the current right personID in the file
