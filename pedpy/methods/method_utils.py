...
def _compute_individual_movement(
    *,
    ...
    if border_method == VelocityBorderMethod.MAXIMUM_RANGE:
        return _compute_movement_max_range(
            traj_data=traj_data,
            frame_step=frame_step,
            bidirectional=bidirectional,
        )
...
def _compute_movement_max_range(
    *, traj_data: pd.DataFrame, frame_step: int, bidirectional: bool = True
    ...
    The movement is computed for the interval [frame - frame_step: frame +
    frame_step], if one of the boundaries is not contained in the trajectory
    frame use the maximum available number of frames on this side and
    frame_step frames on the other.
    ...
    df_movement["start_frame"] = np.maximum(
        df_movement.frame - frame_step, df_movement.frame_min
    )
    df_movement["end_frame"] = np.minimum(
        df_movement.frame + frame_step, df_movement.frame_max
    )
