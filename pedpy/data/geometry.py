...
class WalkableArea:
    ...
    def __setattr__(self, attr, value):
        ...
        """
        if self._frozen:
            raise PedPyAttributeError("Walkable area can not be changed after construction!")
...
class MeasurementArea:
    ...
    def __init__(self, coordinates: Any):
        ...
        try:
            ...
            raise GeometryError(f"Could not create measurement area from the given input: {exc}.") from exc
    ...
    def __setattr__(self, attr, value):
        ...
        """
        if self._frozen:
            raise PedPyAttributeError("Measurement area can not be changed after construction!")
        return super().__setattr__(attr, value)
    ...
    def __init__(self, x_min: float, y_min: float, x_max: float, y_max: float):
        ...
        Raises:
            GeometryError: If the measurement area cannot be created or is
                invalid.
        """
        self._polygon = shapely.box(xmin=x_min, ymin=y_min, xmax=x_max, ymax=y_max)

        if self._polygon.area == 0:
            raise GeometryError("Axis-aligned measurement area needs to cover a non-zero area.")

        shapely.prepare(self._polygon)
        self._frozen = True

    @classmethod
    def from_measurement_area(cls, measurement_area: MeasurementArea) -> "AxisAlignedMeasurementArea":
...
class MeasurementLine:
    ...
    def __init__(self, coordinates: Any):
        ...
        try:
            ...
            raise GeometryError(f"Could not create measurement line from the given coordinates: {exc}.") from exc
        ...
        if len(self._line.coords) != 2:
            raise GeometryError(
                f"Measurement line may only consists of 2 points, but {len(self._line.coords)} points given."
            )
    ...
    def __setattr__(self, attr, value):
        ...
        """
        if self._frozen:
            raise PedPyAttributeError("Measurement line can not be changed after construction!")
        return super().__setattr__(attr, value)
...
def _polygon_from_wkt(wkt_input: str) -> shapely.Polygon:
    try:
        ...
        raise GeometryError(
            f"Could not create geometry objects from the given WKT: {wkt_input}. See following error message:\n{exc}"
        ) from exc
...
def _polygon_from_shapely(
    geometry_input: (shapely.Polygon | shapely.MultiPolygon | shapely.GeometryCollection | shapely.MultiPoint),
) -> shapely.Polygon:
    ...
    if not isinstance(polygon, shapely.Polygon):
        raise GeometryError(
            "Input can not be combined to one single polygon. Make sure, that parts of the input are connected."
        )
    return polygon
...
def _create_polygon_from_input(polygon_input: Any, holes: Optional[List[Any]] = None) -> shapely.Polygon:
    ...
    if isinstance(
        ...
        if holes is not None:
            raise GeometryError("If polygon is of type shapely.Polygon additional holes are not allowed.")
        ...
        try:
            ...
            raise GeometryError(f"Could not create polygon from the given input: {polygon_input}.") from exc
