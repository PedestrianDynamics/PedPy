...
def compute_n_t(
    ...
    crossing_frames = crossing_frames.groupby("ID")["frame"].min().sort_values().reset_index()

    nt = crossing_frames.groupby("frame")["frame"].size().cumsum().rename("Cumulative pedestrians")
