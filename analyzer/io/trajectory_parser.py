...
def parse_trajectory(
    *,
    ...
    Args:
        ...
        frame_rate (float): frame rate of the file, None if frame rate from
    ...
    return TrajectoryData(
        traj_dataframe, traj_frame_rate, traj_type, trajectory_file
    )
...
def parse_trajectory_file(
    *,
    ...
    """Parse the trajectory file for the relevant data: trajectory data, frame
...
def parse_trajectory_data(
    ...
    """
    unit = (
        parse_unit_of_coordinates(trajectory_file)
...
def parse_frame_rate(
    trajectory_file: pathlib.Path, default_frame_rate: float = None
) -> float:
    """Parse the trajectory file for the used framerate.

    Searches for the first line starting with '#' and containing the word
    'framerate' and at least one floating point value. If float values are
...
def parse_trajectory_type(trajectory_file: pathlib.Path) -> TrajectoryType:
    """Parse the trajectory file for the type of trajectory, e.g., the origin
...
def parse_unit_of_coordinates(trajectory_file: pathlib.Path) -> TrajectoryUnit:
    ...
    Returns:
        The unit used in the trajectory file for the coordinates. If no
        explicit unit is given, METER is returned.
    """
