...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            compute_mean_speed_per_frame,
            dict(traj_data=_BAD, individual_speed=_DF, measurement_area=None),
            id="compute_mean_speed_per_frame",
        ),
        pytest.param(
            compute_voronoi_speed,
            dict(traj_data=_BAD, individual_speed=_DF, individual_voronoi_intersection=_DF, measurement_area=None),
            id="compute_voronoi_speed",
        ),
        pytest.param(
            compute_species,
            dict(trajectory_data=_BAD, individual_voronoi_polygons=_DF, measurement_line=None, frame_step=5),
            id="compute_species",
        ),
        pytest.param(
            compute_individual_acceleration, dict(traj_data=_BAD, frame_step=5), id="compute_individual_acceleration"
        ),
        pytest.param(
            compute_mean_acceleration_per_frame,
            dict(traj_data=_BAD, individual_acceleration=_DF, measurement_area=None),
            id="compute_mean_acceleration_per_frame",
        ),
        pytest.param(
            compute_voronoi_acceleration,
            dict(
                traj_data=_BAD, individual_acceleration=_DF, individual_voronoi_intersection=_DF, measurement_area=None
            ),
            id="compute_voronoi_acceleration",
        ),
        pytest.param(
            compute_classic_density, dict(traj_data=_BAD, measurement_area=None), id="compute_classic_density"
        ),
        ...
        pytest.param(
            compute_frame_range_in_area,
            dict(traj_data=_BAD, measurement_line=None, width=1.0),
            id="compute_frame_range_in_area",
        ),
        pytest.param(compute_neighbor_distance, dict(traj_data=_BAD, neighborhood=_DF), id="compute_neighbor_distance"),
        pytest.param(
            compute_time_distance_line, dict(traj_data=_BAD, measurement_line=None), id="compute_time_distance_line"
        ),
        pytest.param(
            compute_individual_voronoi_polygons,
            dict(traj_data=_BAD, walkable_area=None),
            id="compute_individual_voronoi_polygons",
        ),
        pytest.param(
            compute_crossing_frames, dict(traj_data=_BAD, measurement_line=None), id="compute_crossing_frames"
        ),
        pytest.param(
            compute_pair_distribution_function,
            dict(traj_data=_BAD, radius_bin_size=0.1),
            id="compute_pair_distribution_function",
        ),
    ],
)
